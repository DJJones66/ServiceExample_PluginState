#!/usr/bin/env python3
"""
ServiceExample_PluginState Plugin Lifecycle Manager (New Architecture)

This script handles install/update/delete operations for the ServiceExample_PluginState plugin
...
try:
    ...
    try:
        ...
        raise ImportError("ServiceExample_PluginState plugin requires the new architecture BaseLifecycleManager")


class ServiceExamplePluginStateLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for ServiceExample_PluginState plugin using new architecture"""
    
    def __init__(self, plugins_base_dir: str = None):
        """Initialize the lifecycle manager"""
        # Define plugin-specific data for PluginState service bridge example
        self.plugin_data = {
            "name": "ServiceExample_PluginState",
            "description": "Simple example demonstrating PluginState Service usage for state persistence",
            "version": "1.0.0",
            "type": "frontend",
            "icon": "Save",
            "category": "examples",
            ...
            "compatibility": "1.0.0",
            "scope": "ServiceExamplePluginState",
            "bundle_method": "webpack",
            "bundle_location": "dist/remoteEntry.js",
            "is_local": False,
            "long_description": "A focused example demonstrating how to use the PluginState Service Bridge in BrainDrive plugins. Shows state configuration, save/restore operations, validation, and error handling patterns.",
            "plugin_slug": "ServiceExample_PluginState",
            # Update tracking fields (matching plugin model)
            "source_type": "github",
            "source_url": "https://github.com/DJJones66/ServiceExample_PluginState",
            "update_check_url": "https://api.github.com/repos/DJJones66/ServiceExample_PluginState/releases/latest",
            "last_update_check": None,
            "update_available": False,
            "latest_version": None,
            "installation_type": "remote",
            "permissions": ["pluginState.access"]
        }
        
        # Define module data - single focused component
        self.module_data = [
            {
                "name": "PluginStateDemo",
                "display_name": "Plugin State Demo",
                "description": "Complete demonstration of PluginState service bridge usage",
                "icon": "Save",
                "category": "examples",
                "priority": 1,
                "props": {
                    "title": "PluginState Service Demo",
                ...
                "config_fields": {
                    "title": {
                        ...
                        "default": "PluginState Service Demo"
                    },
                    "description": {
                        "type": "text",
                        "description": "Demo component description",
                        "default": "Learn how to use the PluginState service bridge"
                    },
                    "show_debug_info": {
                        "type": "boolean",
                        "description": "Show debug information and state details",
                        "default": True
                    },
                    "auto_save": {
                        "type": "boolean",
                ...
                "tags": ["pluginstate", "service-bridge", "example", "state-management"]
            }
        ]
        
        # Initialize base class with required parameters
        logger.info(f"ServiceExample_PluginState: plugins_base_dir - {plugins_base_dir}")
        if plugins_base_dir:
        ...
        logger.info(f"ServiceExample_PluginState: shared_path - {shared_path}")
        super().__init__(
    ...
    async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_PluginState: User installation completed for {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ServiceExample_PluginState: User installation failed for {user_id}: {e}")
    ...
    async def _perform_user_uninstallation(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_PluginState: User uninstallation completed for {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"ServiceExample_PluginState: User uninstallation failed for {user_id}: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _copy_plugin_files_impl(self, user_id: str, target_dir: Path, update: bool = False) -> Dict[str, Any]:
        """
        ServiceExample_PluginState-specific implementation of file copying.
        ...
        try:
            ...
            logger.info(f"ServiceExample_PluginState: Copied {len(copied_files)} files/directories to {target_dir}")
            return {'success': True, 'copied_files': copied_files}
            
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Error copying plugin files: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        ServiceExample_PluginState-specific validation logic.
        This method is called by the base class during installation.
        """
        try:
            # Check for ServiceExample_PluginState-specific required files
            ...
            if missing_files:
                return {
                    'valid': False,
                    'error': f"ServiceExample_PluginState: Missing required files: {', '.join(missing_files)}"
                }
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'ServiceExample_PluginState: package.json missing required field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'ServiceExample_PluginState: Invalid or missing package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'ServiceExample_PluginState: Bundle file (remoteEntry.js) is empty'
                }
            
            logger.info(f"ServiceExample_PluginState: Installation validation passed for user {user_id}")
            return {'valid': True}
            
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Error validating installation: {e}")
            return {'valid': False, 'error': str(e)}
    
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        ServiceExample_PluginState-specific health check logic.
        ...
        try:
            ...
            logger.error(f"ServiceExample_PluginState: Error checking plugin health: {e}")
            return {
                'healthy': False,
                'details': {'error': str(e)}
            }
    ...
    async def _check_existing_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            plugin_slug = self.plugin_data['plugin_slug']
            logger.info(f"ServiceExample_PluginState: Checking for existing plugin - user_id: {user_id}, plugin_slug: {plugin_slug}")
            
            # First test database connectivity
            test_query = text("SELECT COUNT(*) as count FROM plugin")
            test_result = await db.execute(test_query)
            test_row = test_result.fetchone()
            logger.info(f"ServiceExample_PluginState: Database connectivity test - total plugins: {test_row.count}")
            ...
            logger.info(f"ServiceExample_PluginState: Executing query with params: {query_params}")
            
            result = await db.execute(plugin_query, query_params)
            
            plugin_row = result.fetchone()
            logger.info(f"ServiceExample_PluginState: Query result: {plugin_row}")
            if plugin_row:
                logger.info(f"ServiceExample_PluginState: Found existing plugin - id: {plugin_row.id}, name: {plugin_row.name}")
                return {
                    'exists': True,
                ...
                logger.warning(f"ServiceExample_PluginState: No plugin found for user_id: {user_id}, plugin_slug: {plugin_slug}")
                ...
                if debug_rows:
                    logger.info(f"ServiceExample_PluginState: User has {len(debug_rows)} other plugins:")
                    for row in debug_rows:
                        logger.info(f"  - {row.plugin_slug} (id: {row.id})")
                else:
                    logger.info(f"ServiceExample_PluginState: User has no plugins installed")
                
                return {'exists': False}
                
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Error checking existing plugin: {e}")
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_PluginState: Creating database records - user_id: {user_id}, plugin_slug: {plugin_slug}, plugin_id: {plugin_id}")
            ...
            await db.commit()
            logger.info(f"ServiceExample_PluginState: Database transaction committed successfully")
            ...
            if verify_row:
                logger.info(f"ServiceExample_PluginState: Successfully created and verified database records for plugin {plugin_id} with {len(modules_created)} modules")
            else:
                logger.error(f"ServiceExample_PluginState: Plugin creation appeared to succeed but verification failed for plugin_id: {plugin_id}")
    ...
    async def _export_user_data(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ServiceExample_PluginState: Exported user data for {user_id}")
            return {'success': True, 'user_data': user_data}
            
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Error exporting user data: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _import_user_data(self, user_id: str, db: AsyncSession, user_data: Dict[str, Any]):
        ...
        try:
            if not user_data:
                logger.info(f"ServiceExample_PluginState: No user data to import for {user_id}")
                return
            ...
            logger.info(f"ServiceExample_PluginState: Imported user data for {user_id}")
            
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Error importing user data: {e}")
            raise
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Install ServiceExample_PluginState plugin for specific user (compatibility method)"""
        try:
            logger.info(f"ServiceExample_PluginState: Starting installation for user {user_id}")
            
            # Check if plugin is already installed for this user
            existing_check = await self._check_existing_plugin(user_id, db)
            if existing_check['exists']:
                logger.warning(f"ServiceExample_PluginState: Plugin already installed for user {user_id}")
                return {
                    'success': False,
            ...
            logger.info(f"ServiceExample_PluginState: Created shared directory: {shared_path}")

            # Copy plugin files to the shared directory first
            copy_result = await self._copy_plugin_files_impl(user_id, shared_path)
            if not copy_result['success']:
                logger.error(f"ServiceExample_PluginState: File copying failed: {copy_result.get('error')}")
                return copy_result

            logger.info(f"ServiceExample_PluginState: Files copied successfully, proceeding with database installation")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if not verify_check['exists']:
                        logger.error(f"ServiceExample_PluginState: Installation appeared successful but verification failed")
                        return {'success': False, 'error': 'Installation verification failed'}
                    
                    logger.info(f"ServiceExample_PluginState: Installation verified successfully for user {user_id}")
                    result.update({
                        'plugin_slug': self.plugin_data['plugin_slug'],
                        'plugin_name': self.plugin_data['name']
                    })
                else:
                    logger.error(f"ServiceExample_PluginState: Database installation failed: {result.get('error')}")
                
                return result
                
            except Exception as db_error:
                logger.error(f"ServiceExample_PluginState: Database operation failed: {db_error}")
            ...
            logger.error(f"ServiceExample_PluginState: Install plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def delete_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Delete ServiceExample_PluginState plugin for user (compatibility method)"""
        try:
            logger.info(f"ServiceExample_PluginState: Starting deletion for user {user_id}")
            ...
            if result.get('success'):
                logger.info(f"ServiceExample_PluginState: Successfully deleted plugin for user {user_id}")
            else:
                logger.error(f"ServiceExample_PluginState: Deletion failed: {result.get('error')}")
            
            return result
        except Exception as e:
            logger.error(f"ServiceExample_PluginState: Delete plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Get current status of ServiceExample_PluginState plugin installation (compatibility method)"""
        try:
            existing_check = await self._check_existing_plugin(user_id, db)
            if not existing_check['exists']:
            ...
            logger.error(f"ServiceExample_PluginState: Error checking plugin status: {e}")
            return {'exists': False, 'status': 'error', 'error': str(e)}
    
    async def update_plugin(self, user_id: str, db: AsyncSession, new_version_manager: 'ServiceExamplePluginStateLifecycleManager') -> Dict[str, Any]:
        """Update ServiceExample_PluginState plugin for user (compatibility method)"""
        try:
            ...
            logger.info(f"ServiceExample_PluginState: Plugin updated successfully for user {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ServiceExample_PluginState: Plugin update failed for user {user_id}: {e}")
            return {'success': False, 'error': str(e)}
...
async def install_plugin(user_id: str, db: AsyncSession, plugins_base_dir: str = None) -> Dict[str, Any]:
    manager = ServiceExamplePluginStateLifecycleManager(plugins_base_dir)
...
async def update_plugin(user_id: str, db: AsyncSession, new_version_manager: 'ServiceExamplePluginStateLifecycleManager', plugins_base_dir: str = None) -> Dict[str, Any]:
    old_manager = ServiceExamplePluginStateLifecycleManager(plugins_base_dir)
...
if __name__ == "__main__":
    ...
    async def main():
        print("ServiceExample_PluginState Plugin Lifecycle Manager - Test Mode")
        print("=" * 50)
        
        # Test manager initialization
        manager = ServiceExamplePluginStateLifecycleManager()
